...
public abstract class MyPipelineBase extends Queable implements MyBinaryRedisPipeline, MyRedisPipeline {

    protected abstract MyClient getClient(String key);

    protected abstract MyClient getClient(byte[] key);
    ...
    public Response<Long> linsert(String key, MyBinaryClient.LIST_POSITION where, String pivot, String value) {
        getClient(key).linsert(key, where, pivot, value);
        return getResponse(BuilderFactory.LONG);
    }

    public Response<Long> linsert(byte[] key, MyBinaryClient.LIST_POSITION where, byte[] pivot, byte[] value) {
        getClient(key).linsert(key, where, pivot, value);
        return getResponse(BuilderFactory.LONG);
    }
